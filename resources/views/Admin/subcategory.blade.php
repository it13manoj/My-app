...
<div class="content-wrapper">
 ...
 <!-------------Country Listing Start From Here--------------->
            <div class="card-body" id="listingdiv">
            	<button type="button" id="addbutton" class="btn btn-warning btn-fw" style="float:right;"> <span>+ Add Subcategory</span></button>
              <h4 class="card-title">Subcategory Manager</h4>
              ...
              <div class="row">
                <div class="col-12">
                  <div class="table-responsive">
                    <table id="scatTable" class="table">
                      <thead>
                        <tr>
                            <th> #</th>
            ...
            <div class="card-body" style="display: none;padding: 1.88rem" id="addeditdiv">
              ...
              <div class="row">
                <div class="col-8">
                  <div class="table-responsive">
                    <form class="forms-sample dataformtosubmit" method="POST" action="{{route('manager.jobsubcategories.submit')}}">
                     @csrf
                    <div class="form-group">
                      <label for="exampleInputName1">Subcategory Name</label>
                      <input class="form-control" required name="scatname" id="scatname" placeholder="Subcategory Name" type="text">
                    </div>
                    <div class="form-group">
        ...
        $(document).on('click','.editbtn',function(){
            var id=$(this).attr('id');
             $(".loader").show();
            $.ajax({
                   url : "{{route('manager.get_jobsubcategory_details')}}",
                   method : "GET",
                   data : {id:id},
                  dataType:'json',
                   success : function (data)
                   {
                      $("#scatname").val(data.subcategory_name);
                       $("#category").val(data.category_id);
                      $("#scid").val(data.subcategory_id);
                       $("#listingdiv").hide();
                       $("#addeditdiv").slideDown( "slow", function() {
                                // Animation complete.
                              });
            ...
            var id=$(this).attr('idd');
             ...
             swal({
                ...
                $.ajax({
                   ...
                   method : "POST",
                   data : {id:id,status_of:'subcategory','_token':'{{ csrf_token() }}'},
                  dataType:'text',
                   success : function (data)
                   {
                      if(data==1){
